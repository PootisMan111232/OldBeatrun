...
local function HumanCount()
	...
	for k,v in ipairs(player.GetAll()) do
		if IsValid(v) and !v:GetNW2Bool("Infected") then
			count = count+1
		end
	end
	return count
...
if SERVER then
	...
	local function GiveLastManGun()
		if cachedhumancount == 1 then
			for k,v in pairs(player.GetAll()) do
				if !didgun and !ended and v:Alive() and !v:GetNW2Bool("Infected") then
					-- v:Give("weapon_357")
					-- v:GiveAmmo(6, "357")
					didgun = true
					break
	...
	net.Receive("Infection_Touch", function(len, ply)
		...
		if ended or !ply:Alive() or (!ply:GetNW2Bool("Infected") or victim:GetNW2Bool("Infected")) then return end
		if IsValid(victim) and victim:IsPlayer() then
			if ply:GetPos():Distance(victim:GetPos()) < 300 then
				print(ply, "Closed distance with", victim)
	...
	local function InfectionSync(ply)
		if GetGlobalBool(GM_INFECTION) and !ply.InfectionSynced then
	...
	local function InfectionTimer()
		...
		if !revealed and CurTime() >= Infection_StartTime then
			revealed = true
			Beatrun_FirstInfection()
		end
		
		local timeremaining = Infection_EndTime-CurTime()
		if !didmusic and revealed and (timeremaining <= 60 and timeremaining >= 50) and player.GetCount() >= 5 and cachedhumancount == 1 then
			timer.Simple(0.1, function()
			for k,v in ipairs(player.GetAll()) do
				if v:Alive() and !v:GetNW2Bool("Infected") then
		...
		if CurTime() >= Infection_EndTime and !ended then
			for k,v in ipairs(player.GetAll()) do
				if v:GetNW2Float("PBTime") == 0 and !v:GetNW2Bool("Infected") then
	...
	function Beatrun_StartInfection()
		if GetGlobalBool(GM_INFECTION) and !ended then return end
		...
		for k,v in ipairs(players) do
			...
			v:StripAmmo()
			-- timer.Simple(0, function() v:StripWeapon("weapon_357") end)
	...
	function InfectionDeath(ply)
		if !GetGlobalBool(GM_INFECTION) then return end
		if revealed and CurTime() > Infection_StartTime and !ply:GetNW2Bool("Infected") then
...
if CLIENT then
	...
	local function InfectionCalcView(ply, pos, ang)
		if GetGlobalBool(GM_INFECTION) then
			...
			if keydown and !IsValid(VMLegs.LegModel) and !IsValid(BodyAnim) then
				ang.x = 0
				ang.z = 0
				ang.y = ang.y+180
				view.origin = pos
			...
			if lookingbehind and !keydown then
				lookingbehind = false
	...
	net.Receive("Infection_Start", function()
		...
		noclipkey = input.GetKeyCode(input.LookupBinding("noclip"))
		endtime = 0
	...
	local function InfectionHUD()
		...
		if Infection_StartTime > CurTime() then
	...
	local tab = {
		[ "$pp_colour_addr" ] = 32/255,
		[ "$pp_colour_addg" ] = 6/255,
		[ "$pp_colour_addb" ] = 6/255,
