...
local function HideNearby(ply)
	...
	if ( Distance < 20000 ) then
		...
		if (ply.distfromlocal or 100) < 30 then
			if infectionmode and CurTime() > (LocalPlayer().InfectionTouchDelay or 0) and localinfected and !plyinfected then
		...
		if infectionmode and (localinfected and !plyinfected) or (!localinfected and plyinfected) then
			color = enemy
